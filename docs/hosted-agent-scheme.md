...
- `vercel.json` schedules `/api/agent/worker/run?limit=1` once per day by default because Vercel Hobby only allows daily Cron Jobs. For a responsive background worker, upgrade the Vercel project or call the worker endpoint from an external cron every few minutes.
- Set `CRON_SECRET` or `AGENT_WORKER_SECRET` in Vercel so the worker endpoint is authorized in hosted deployments.
