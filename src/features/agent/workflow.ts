...
import type { AgentAction, AgentAuditSnapshot, AgentChange, AgentLiveItem, BuildMode, FileBuffer } from "../../shared/types";
...
export async function runAgentWorkflow({
  ...
  const setAssistantLiveText = (text: string, tone: AgentLiveItem["tone"] = "default", meta: string | null = null) => {
    ...
    setAgentLiveItems((prev) => {
      ...
      for (let index = next.length - 1; index >= 0; index -= 1) {
        if (next[index].role === "assistant") {
          next[index] = { ...next[index], tone, text: clean, meta };
          return next;
        }
      }
      next.push({ id: makeAgentLiveId(), role: "assistant", tone, text: clean, meta });
      return next;
    });
  };
      ...
      (event) => {
          ...
          if (phase && !seenPhases.has(phase)) {
            ...
            if (phase === "queued" && jobId) {
              pushAgentLiveItem({
                ...
                kind: "status",
                data: { phase, job_id: jobId, message },
              });
            }
          }
          return;
        ...
        if (event.event === "tool_call") {
          ...
          pushAgentLiveItem({
            ...
            meta: summary || phase,
            kind: "tool_call",
            data: { ...event.data, name, phase, summary },
          });
          return;
        }
        ...
        if (event.event === "tool_output") {
          ...
          pushAgentLiveItem({
            ...
            kind: "tool_output",
            data: { ...event.data, name, ok, duration, error },
          });
          return;
        }
  ...
  const applyChangesWithConflictRepair = async (
    ...
    for (let pass = 0; pass <= maxConflictRepairPasses; pass += 1) {
      try {
        ...
        if (repairRes.spoken) {
          setAgentReply(repairRes.spoken);
          setAssistantLiveText(repairRes.spoken);
        }
    ...
    for (const action of actions) {
      ...
      try {
        pushAgentLiveItem({
          role: "tool",
          tone: "working",
          text: "Aku serahin command ke backend harness biar policy, cwd, dan sync file ditangani satu tempat.",
          meta: action.command,
          kind: "shell",
          data: { command: action.command, reason, state: "running" },
        });
        ...
        pushAgentLiveItem({
          ...
          data: {
            ...
            stdout: runRes.stdout,
            stderr: runRes.stderr,
            synced_files: runRes.synced_files,
            policy: runRes.policy,
            reason,
            state: "done",
          },
        });
        ...
        pushAgentLiveItem({
          role: "tool",
          tone: "error",
          text: "Ada command yang gagal dieksekusi.",
          meta: message,
          kind: "shell",
          data: { command: action.command, ok: false, error: message, reason, state: "error" },
        });
      }
    }
  ...
  const refreshPreviewSurface = async () => {
    ...
    pushAgentLiveItem({
      ...
      kind: "preview",
      data: { url: currentPreviewUrl || null, state: currentPreviewUrl ? "refreshing" : "starting" },
    });
  ...
  const runValidationPass = async (label: string) => {
    ...
    pushAgentLiveItem({
      ...
      data: {
        label,
        ok: validation.ok,
        ran: validation.results.length,
        failed: validation.results.filter((result) => !result.ok).length,
  ...
  try {
    ...
    setAssistantLiveText(res.spoken);
    setAgentLog(res.log);
    ...
    if (
      ...
      if (verifierRepairRes.spoken) {
        setAgentReply(verifierRepairRes.spoken);
        setAssistantLiveText(verifierRepairRes.spoken);
      }
    ...
    if (backendAutoExecuted) {
      ...
      setAssistantLiveText(finalBubble.text, finalBubble.tone, finalBubble.meta);
    }
    ...
    setAssistantLiveText(`Agent gagal: ${message}`, "error");
    notify({ kind: "error", message: `Agent error: ${message}` });
  } finally {
    setWorkingMsg("");
  }
}
