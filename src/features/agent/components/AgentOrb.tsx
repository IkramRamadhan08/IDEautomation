...
import { getBuildModeProfile, getModeQuickPrompts } from "../runtime";
import { AgentLiveStage } from "./AgentLiveStage";
import { type AgentLiveItem, type BuildMode, type UploadedImageAsset } from "../../../shared/types";
...
export const AgentOrb: React.FC<AgentOrbProps> = ({
  ...
  const canRunAgent = agentInput.trim().length > 0 && agentStatus !== "thinking";
  ...
  const visibleToolItems = agentLiveItems.filter((item) => item.role === "tool");
  ...
  const syncPromptHeight = React.useCallback((node: HTMLTextAreaElement | null) => {
    if (!node) return;
  ...
  React.useEffect(() => {
    if (!agentWidgetOpen || agentStatus === "thinking") return;
    const frame = window.requestAnimationFrame(() => {
      promptRef.current?.focus();
      syncPromptHeight(promptRef.current);
    });
    return () => window.cancelAnimationFrame(frame);
  }, [agentStatus, agentWidgetOpen, showRunExperience, syncPromptHeight]);

  React.useEffect(() => {
    syncPromptHeight(promptRef.current);
  }, [agentInput, syncPromptHeight]);
  ...
  const renderPromptComposer = (placeholder: string, showAssetHint = false, disabled = false) => (
    <>
      <div className={`agentOrbComposer ${agentInput.trim() ? "hasText" : ""} ${disabled ? "disabled" : ""}`}>
        <textarea
          ref={promptRef}
        ...
        />
        <div className="agentOrbComposerMeta">
          {disabled ? (
            <>
              <span>{workingMsg || "Agent lagi kerja"}</span>
              <span>Follow-up aktif setelah run selesai</span>
            </>
          ) : (
            <>
              <span>{agentInput.trim() ? `${agentInput.trim().length} chars` : "Enter to run"}</span>
      ...
      {attachedImage ? (
        <div className="attachedImageChip agentOrbAssetChip">
          ...
          {showAssetHint ? <span className="attachedImageHint">pakai di prompt, contoh: {attachedImageToken || "@image"} sebagai hero</span> : null}
  ...
  const renderPromptActions = (disabled = false) => (
      <div className="agentOrbActions">
        <button className="btn subtleBtn" onClick={onPickAgentImage} disabled={disabled || imageUploading}>
          <Paperclip size={14} />
          <span>{imageUploading ? "Uploading..." : "Attach"}</span>
        </button>
        <button className="btn primary" onClick={onRunAgent} disabled={disabled || !canRunAgent}>
          <SendHorizontal size={14} />
          <span>{disabled ? "Working" : "Run"}</span>
        </button>
      </div>
  );
                ...
                {showRunExperience ? (
                  <>
                    <div className="agentOrbRunHeader">
                      <div>
                        <div className="agentOrbSectionLabel">Conversation</div>
                    ...
                    {visibleToolItems.length > 0 ? (
                      <div className="agentOrbActivityPanel">
                        <div className="agentOrbActivityHeader">
                          <span>Run activity</span>
                          <span>{agentStatus === "thinking" ? "Live" : "Latest"}</span>
                        </div>
                        <AgentLiveStage
                    ...
                    {renderPromptComposer(
                      agentStatus === "thinking" ? "Agent lagi jalan. Follow-up bisa diketik setelah selesai..." : "Tulis follow-up, minta revisi, atau kasih task berikutnya...",
                      false,
                      agentStatus === "thinking",
                    )}
                    {renderPromptActions(agentStatus === "thinking")}
                  </>
                ) : (
                  <>
                    {renderPromptComposer(
                    ...
                    {!agentInput.trim() ? <div className="agentOrbQuickGrid">
